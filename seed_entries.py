...
with open("C:/Users/YOGA/AppData/Local/Temp/dd_state.json") as f:
    state = json.load(f)
TOKEN = state["token"]
storage_paths = state["storage_paths"]
...
entries_data = [
    {
        ...
        "image_idx": 0, "location": "Home, Mumbai"
    },
    {
        ...
        "image_idx": 1, "location": "Family Home, Pune"
    },
    {
        ...
        "image_idx": 2, "location": "Cubbon Park, Bangalore"
    },
    {
        ...
        "image_idx": 3, "location": "Office, Bangalore"
    },
    {
        ...
        "image_idx": 4, "location": "Rooftop, Bangalore"
    },
    {
        ...
        "image_idx": 5, "location": "Home, Bangalore"
    },
    {
        ...
        "image_idx": 6, "location": "Indiranagar, Bangalore"
    },
    {
        ...
        "image_idx": 7, "location": "Gym, Bangalore"
    },
    {
        ...
        "image_idx": 8, "location": "MG Road, Bangalore"
    },
    {
...
print("Step 3: Inserting 10 journal entries...")
entry_ids = []
for i, e in enumerate(entries_data):
    ...
    payload = {
        "id": entry_id,
        "user_id": USER_ID,
        "encrypted_content": e["content"],
        "encrypted_raw_content": e["raw"],
        "mood": e["mood"],
        "entry_date": e["date"],
        "entry_time": e["time"],
    ...
    if r.status_code == 201:
        ...
        print(f"  FAIL entry {i+1}: {r.status_code} {r.text[:100]}")

print(f"\nStep 3b: Attaching images to entries...")
for idx, (entry_id, e) in enumerate(entry_ids):
    img_idx = e["image_idx"]
    if img_idx < len(storage_paths):
        _, sp = storage_paths[img_idx]
        media_payload = {
            "id": str(uuid.uuid4()),
            "entry_id": entry_id,
            "user_id": USER_ID,
...
print(f"\nStep 4: Calling ai-polish for each entry...")
polished = 0
for entry_id, e in entry_ids:
    try:
        r = requests.post(f"{FN_URL}/ai-polish",
            headers=H,
            json={"entryId": entry_id, "content": e["content"], "mood": e["mood"], "writingStyle": "memoir"},
            timeout=30,
        )
        if r.status_code == 200:
            polished += 1
            resp = r.json()
            chars = len(resp.get("polished", resp.get("content", "")))
...
print(f"\nStep 5: Calling ai-tag for each entry...")
tagged = 0
for entry_id, e in entry_ids:
    try:
        r = requests.post(f"{FN_URL}/ai-tag",
            headers=H,
...
print(f"\nStep 6: Weekly story generation...")
for week_offset in [0, -1]:
    try:
        r = requests.post(f"{FN_URL}/ai-weekly-page",
            headers=H,
...
print(f"  Images attached  : {len(entry_ids)}")
